import React, {memo, PropsWithChildren, useContext, useMemo, useState} from 'react'
...
import { createContext, defineContext, Provider } from '@reconjs/react'
import { FeedPostSlice } from '@/bsky/state/queries/post-feed'
import { FeedRow } from './post-feed'

const theFeed = createContext <string>()
const theRow = createContext <string>()
const thePost = createContext <string>()

const theFeedItems = defineContext (() => {
	const feed = useContext (theFeed)
	const query = useSuspenseQuery <FeedRow[]> ({
		queryKey: [ 'feed', feed ],
		queryFn: () => [],
	})
	return query.data
...
const theRowData = defineContext (() => {
	const items = useContext (theFeedItems)
	const row = useContext (theRow)
	const data = items.find (item => item.key === row)
	if (!data) throw new Error ('Row not found')
	return data
}, [ theRow, theFeedItems ])

const theSubslice = defineContext (() => {
	const data = useContext (theRowData)
	if (data.type !== 'sliceItem') throw new Error ('Slice expected')
	const { slice, indexInSlice } = data
	return slice.items[indexInSlice]
}, [ theRowData ])

const ITEM = <PostFeedItem />

function FeedItem () {
	const { post } = useContext (theSubslice)

	return (
		<Provider context={thePost} value={post.cid}>
			{ITEM}
		</Provider>
	)
}
